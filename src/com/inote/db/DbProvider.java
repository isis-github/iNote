...
public class DbProvider extends ContentProvider {
	...
	static {
		...
		// NoteItems表
		mUriMatcher.addURI(Db.AUTHORITY, "noteitems", NOTEITEMS);
		mUriMatcher.addURI(Db.AUTHORITY, "noteitems/#", NOTEITEMS_ITEM);
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			String sql_noteitems = "CREATE TABLE " + TABLE_NOTEITEMS + " ("
					...
					+ " INTEGER" + ");";
			ILog.w(MainActivity.TAG, "创建表 " + TABLE_NOTEITEMS + " 的SQL语句："
					+ sql_noteitems);
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			ILog.w(MainActivity.TAG, "Upgrading database from version "
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		case NOTEITEMS:// Uri匹配到NoteItems表
			...
			if (id > 0) {
				...
				ILog.d(MainActivity.TAG, "ContentProvider==>insert()");
				return tmpUri;
			}
			break;
		default:
