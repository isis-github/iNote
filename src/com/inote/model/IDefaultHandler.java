...
public class IDefaultHandler extends DefaultHandler {
	// 用List存储读取到的记录
	private List<INote> mRecords;
	private INote mNote;
	...
	public void characters(char[] ch, int start, int length)
			throws SAXException {
		ILog.e(MainActivity.TAG, "^^^当节点中有换行符时,characters()函数不止执行一次^^^  "
		...
		if (mNote != null) {
			...
			if (preTag.equals("id")) {
				...
				ILog.d(MainActivity.TAG, "读取XML文件时==>characters()函数中的数据: "
						+ strContent);
	...
	public void endElement(String uri, String localName, String qName)
			throws SAXException {
		if (!TextUtils.isEmpty(strElementData)) {
			ILog.d(MainActivity.TAG, "从XML文件中读取到的完整的content节点的内容: "
					+ strElementData);
			ILog.d(MainActivity.TAG, "从XML文件中读取到的完整的content节点的内容打印完毕!");
			// 保存Content到Note对象中
